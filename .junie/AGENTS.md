...
3. Create `pkg/authctx/authctx.go` — the auth context helper from Section 1A.2
4. Create `core/domain/*.go` — all entity structs from Section 4
5. Create `core/domain/errors.go` — all error types from Section 3.3
6. Create `core/ports/*.go` — all interfaces from Section 5
7. Run `go build ./...` — must compile with zero errors
...
```json
{
  ...
  "wire_code": {
    "import": "messaging \"github.com/ranakdinesh/spur-messaging\"",
    "init": "encKey, err := hex.DecodeString(cfg.MessagingEncryptionKey)\nif err != nil { return nil, fmt.Errorf(\"MESSAGING_ENCRYPTION_KEY must be 64-char hex: %w\", err) }\nif len(encKey) != 32 { return nil, fmt.Errorf(\"MESSAGING_ENCRYPTION_KEY must be 32 bytes (64 hex chars)\") }\nmessagingCfg := messaging.Config{\n\tEncryptionKey: encKey,\n\tWebhookBaseURL: cfg.MessagingWebhookBaseURL,\n\tDefaultRateLimit: cfg.GetIntOrDefault(\"MESSAGING_DEFAULT_RATE_LIMIT\", 10),\n\tRedisURL: cfg.MessagingRedisURL,\n\tWorkerCount: cfg.GetIntOrDefault(\"MESSAGING_WORKER_COUNT\", 5),\n\tEmailProvider: cfg.GetOrDefault(\"MESSAGING_EMAIL_PROVIDER\", \"sendgrid\"),\n\tEmailAPIKey: cfg.ResolveEmailAPIKey(),\n\tEmailFromAddress: cfg.GetOrDefault(\"MESSAGING_EMAIL_FROM_ADDRESS\", \"noreply@example.com\"),\n\tEmailFromName: cfg.GetOrDefault(\"MESSAGING_EMAIL_FROM_NAME\", \"Spur\"),\n\tEmailTrackOpens: cfg.GetBoolOrDefault(\"MESSAGING_EMAIL_TRACK_OPENS\", true),\n\tEmailTrackClicks: cfg.GetBoolOrDefault(\"MESSAGING_EMAIL_TRACK_CLICKS\", true),\n\tSMSProvider: cfg.GetOrDefault(\"MESSAGING_SMS_PROVIDER\", \"msg91\"),\n\tSMSAPIKey: cfg.ResolveSMSAPIKey(),\n\tSMSSenderID: cfg.GetOrDefault(\"MESSAGING_SMS_SENDER_ID\", \"SPUR\"),\n}\nmessagingLog := infra.Log.Logger()\nmessagingModule, err := messaging.New(ctx, messaging.Options{DB: infra.DB, Log: &messagingLog, Cfg: messagingCfg})\nif err != nil { return nil, fmt.Errorf(\"messaging: %w\", err) }",
