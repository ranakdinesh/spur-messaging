...
CREATE TABLE messaging.webhook_endpoints (
    id            UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id     UUID NOT NULL,
    url           TEXT NOT NULL,
...
CREATE TABLE messaging.wallet_ledger (
    id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id      UUID NOT NULL,
...
CREATE INDEX idx_wallet_ledger_tenant_currency
...
CREATE TABLE messaging.rate_cards (
    id             UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tenant_id      UUID,
    channel        TEXT NOT NULL CHECK (channel IN ('whatsapp', 'sms', 'email')),
...
CREATE INDEX idx_conversations_inbox ON messaging.conversations (tenant_id, status, updated_at DESC);
CREATE INDEX idx_conversations_assignee ON messaging.conversations (tenant_id, assigned_agent_id, status);
CREATE INDEX idx_conversations_tags ON messaging.conversations USING GIN (tags);
